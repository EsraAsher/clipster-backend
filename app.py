        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            try:
                ...
                if "private video" in error_msg or "sign in" in error_msg:
                    return JSONResponse(
                        {"error": "This video is private or requires authentication. Please try a public video."}, 
                        status_code=400
                    )
                    ...
                    return JSONResponse(
                        {"error": "Live streams cannot be downloaded while they are ongoing."}, 
                        status_code=400
                    )
                    ...
                    return JSONResponse(
                        {"error": f"Cannot access this video: {str(e)}"}, 
                        status_code=400
                    )
            ...
            if not os.path.exists(final_file):
                return JSONResponse(
                    {"error": "Download completed but file not found. Please try again."}, 
                    status_code=500
                )
            ...
            return JSONResponse({
                ...
                "downloadUrl": f"http://127.0.0.1:8000/file/{video_id}.{ext}"
            })
